...
int libipv6calc_db_wrapper_GeoIP2_wrapper_init(void) {
	...
	for (i = 0; i < MAXENTRIES_ARRAY(libipv6calc_db_wrapper_GeoIP2_db_file_desc); i++) {
		...
		DEBUGPRINT_WA(DEBUG_libipv6calc_db_wrapper_GeoIP2, "GEOIP2 type=%d dbym=%d Year/Month=%d unixtime=%ld", dbtype, dbym, GeoIP2_UNPACK_YM(dbym), mmdb_cache[dbtype].metadata.build_epoch);
...
void libipv6calc_db_wrapper_GeoIP2_wrapper_info(char* string, const size_t size) {
	...
	snprintf(string, size, "GeoIP2 available databases: Country4=%d Country6=%d City4=%d City6=%d", (wrapper_features & IPV6CALC_DB_IPV4_TO_CC) ? 1 : 0, (wrapper_features & IPV6CALC_DB_IPV6_TO_CC) ? 1 : 0, (wrapper_features & IPV6CALC_DB_IPV4_TO_CITY) ? 1 : 0, (wrapper_features & IPV6CALC_DB_IPV6_TO_CITY) ? 1 : 0);
	...
	for (type = 0; type < 32 * GeoIP2_DB_MAX_BLOCKS_32; type++) {
		if ((geoip2_db_usage_map[type / 32] & (1 << (type % 32))) != 0) {
			...
			if (strlen(geoip2_db_usage_string) > 0) {
				...
				snprintf(tempstring, sizeof(tempstring), "%s / %s", geoip2_db_usage_string, info);
			} else {
...
static char *libipv6calc_db_wrapper_GeoIP2_dbfilename(const unsigned int type) {
	...
	snprintf(tempstring, sizeof(tempstring), "%s/%s", geoip2_db_dir, libipv6calc_db_wrapper_GeoIP2_db_file_desc[i].filename);
...
*/
	...
	DEBUGPRINT_WA(DEBUG_libipv6calc_db_wrapper_GeoIP2, "meta: NodeCount=%i RecordSize=%i IpVersion=%i BinaryFormat=%i.%i BuildTime=%lu Type=%s", \
	...
	snprintf(resultstring, sizeof(resultstring), "GeoIP2-%d/%s %s Copyright (c) %s MaxMind All Rights Reserved"
		, type
		, mmdb.metadata.database_type
		, (type <= 2) ? "FREE" : "COMM"
		, year
...
/*
 * wrapper: GeoIP2_get_country_short
 */
char *libipv6calc_db_wrapper_GeoIP2_get_country_short(DB *dbp, char *ip) {
	DEBUGPRINT_WA(DEBUG_libipv6calc_db_wrapper_GeoIP2, "Called: %s ip=%s", wrapper_geoip2_info, ip);

	// not implemented so far
	if (dbp == NULL) { }; // make compiler happy (avoid unused "...")
	//return(GeoIP2_get_country_short(loc, ip));
	return(NULL);
};
