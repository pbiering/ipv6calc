...
int libipv6calc_db_wrapper_DBIP2_wrapper_init(void) {
	...
	for (i = 0; i < MAXENTRIES_ARRAY(libipv6calc_db_wrapper_DBIP2_db_file_desc); i++) {
		...
		DEBUGPRINT_WA(DEBUG_libipv6calc_db_wrapper_DBIP2, "DBIP2 type=%d dbym=%d Year/Month=%d unixtime=%ld", dbtype, dbym, DBIP2_UNPACK_YM(dbym), mmdb_cache[dbtype].metadata.build_epoch);
...
void libipv6calc_db_wrapper_DBIP2_wrapper_info(char* string, const size_t size) {
	...
	snprintf(string, size, "DBIP2 available databases: Country4=%d Country6=%d City4=%d City6=%d", (wrapper_features & IPV6CALC_DB_IPV4_TO_CC) ? 1 : 0, (wrapper_features & IPV6CALC_DB_IPV6_TO_CC) ? 1 : 0, (wrapper_features & IPV6CALC_DB_IPV4_TO_CITY) ? 1 : 0, (wrapper_features & IPV6CALC_DB_IPV6_TO_CITY) ? 1 : 0);
...
void libipv6calc_db_wrapper_DBIP2_wrapper_print_db_info(const int level_verbose, const char *prefix_string) {
	...
	for (i = 0; i < MAXENTRIES_ARRAY(libipv6calc_db_wrapper_DBIP2_db_file_desc); i++) {
		...
		if (libipv6calc_db_wrapper_DBIP2_db_avail(type)) {
			...
			if (ret != MMDB_SUCCESS) {
				fprintf(stderr, "%sDBIP2: %-30s: %-30s (CAN'T OPEN database information)\n", prefix, libipv6calc_db_wrapper_DBIP2_db_file_desc[i].description, libipv6calc_db_wrapper_DBIP2_dbfilename(type));
			} else {
				fprintf(stderr, "%sDBIP2: %-30s: %-30s (%s)\n", prefix, libipv6calc_db_wrapper_DBIP2_db_file_desc[i].description, libipv6calc_db_wrapper_DBIP2_db_file_desc[i].filename, libipv6calc_db_wrapper_DBIP2_database_info(type));
				libipv6calc_db_wrapper_DBIP2_close(type);
				count++;
			};
		} else {
			if (level_verbose == LEVEL_VERBOSE2) {
				fprintf(stderr, "%sDBIP2: %-30s: %-30s (%s)\n", prefix, libipv6calc_db_wrapper_DBIP2_db_file_desc[i].description, libipv6calc_db_wrapper_DBIP2_dbfilename(type), strerror(errno));
			};
			continue;
		};
	};
...
*/
	...
	DEBUGPRINT_WA(DEBUG_libipv6calc_db_wrapper_DBIP2, "meta: NodeCount=%i RecordSize=%i IpVersion=%i BinaryFormat=%i.%i BuildTime=%lu Type=%s", \
