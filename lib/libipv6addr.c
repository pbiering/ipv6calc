 ...
 * Project    : ipv6calc
 * File       : libipv6addr.c
 * Version    : $Id: libipv6addr.c,v 1.72 2013/02/24 13:31:29 ds6peter Exp $
...
uint32_t ipv6addr_checksum_anonymized_iid(const ipv6calc_ipv6addr *ipv6addrp) {
	...
	for (i = 0; i <= 1; i++) {
		for (n = 0; n <= 7; n++) {
			if ((i == 1) && (n == 0)) {
				// last nibble, do not count
			} else {
				checksum += (iid[i] & (0xf << (n*4))) >> (n*4);
			};
		};
	};

	checksum &= 0xf; // limit to 4 bits
 ...
 *
 * in:  ipv6addrp  = pointer to IPv6 address structure
 * out: 0=probably generated by privacy extension, 1=manual set, -1=global, 2=already anonymized privacy extension, 3=already anonymized static
 */
int ipv6addr_privacyextensiondetection(const ipv6calc_ipv6addr *ipv6addrp, s_iid_statistics *iid_statisticsp) {
	...
	if ((iid[0] == ANON_RFC3041_IID_00_31) && (iid[1] == ANON_RFC3041_IID_32_63)) {
		if ( (ipv6calc_debug & DEBUG_libipv6addr_privacydetection) != 0 ) {
			fprintf(stderr, "%s/%s: anonymized privacy extension IID (no further privacy detection)\n", __FILE__, __func__);
		};
		return (2);
	};

	if ((iid[0] == ANON_STATIC_IID_00_31) && (iid[1] == ANON_STATIC_IID_32_63)) {
		if ( (ipv6calc_debug & DEBUG_libipv6addr_privacydetection) != 0 ) {
			fprintf(stderr, "%s/%s: anonymized static IID (no further privacy detection)\n", __FILE__, __func__);
		};
		return (3);
	};
	...
	if ( ( ((typeinfo & (IPV6_ADDR_LINKLOCAL | IPV6_ADDR_SITELOCAL | IPV6_NEW_ADDR_AGU | IPV6_ADDR_ULUA )) != 0) || ((typeinfo & (IPV6_ADDR_LOOPBACK | IPV6_NEW_ADDR_SOLICITED_NODE)) == (IPV6_ADDR_LOOPBACK | IPV6_NEW_ADDR_SOLICITED_NODE)) ) && ((typeinfo & (IPV6_NEW_ADDR_TEREDO | IPV6_NEW_ADDR_ORCHID)) == 0) ) {
		if (ipv6addr_getoctet(ipv6addrp, 11) == 0xff && ipv6addr_getoctet(ipv6addrp, 12) == 0xfe) {
			...
			if ( (typeinfo & IPV6_NEW_ADDR_IID_PRIVACY) == IPV6_NEW_ADDR_IID_PRIVACY ) {
				...
				// OLD ipv6addr_setdword(ipv6addrp, 2, ANON_RFC3041_IID_00_31);
				// OLD ipv6addr_setdword(ipv6addrp, 3, ANON_RFC3041_IID_32_63);
				...
				if ( (typeinfo & IPV6_NEW_ADDR_SOLICITED_NODE) != 0 ) {
					...
					if ((typeinfo & IPV6_NEW_ADDR_6TO4_MICROSOFT) != 0) {
						...
						if (mask_iid == 1) {
							/* mask IID with special value */
							ipv6addr_setdword(ipv6addrp, 2, ANON_STATIC_IID_00_31);
							ipv6addr_setdword(ipv6addrp, 3, ANON_STATIC_IID_32_63);
						} else {
