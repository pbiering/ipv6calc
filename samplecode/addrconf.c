 ...
 *
 *	$Id: addrconf.c,v 1.1.1.1 2001/10/07 14:47:40 peter Exp $
 *
...
#if HZ == 100
#define timeticks(_h)	(_h)
#else
#define timeticks(_h)	((unsigned long)(((unsigned long long)(_h))*100/HZ))
#endif
 ...
 * from ip6_fib.c
 */
static __inline__ int inet6_addr_diff(void *token1, void *token2)
{
	__u32 *a1 = token1;
	__u32 *a2 = token2;
	int addrlen = 4;
...
ok:
		...
		if (ifp) {
			int event = 0;
#ifdef CONFIG_IPV6_ACONF_MIN_VALID_LFT
		    ...
		    void * data)
{
	...
	case NETDEV_UP:
	{
		struct inet6_dev *idev = ipv6_get_idev(dev);
		switch(dev->type) {
		case ARPHRD_SIT:
			/* XXX idev = no katachini */
		...
		if (idev)
			idev->stats.ipv6.Ip6LastChange = timeticks(jiffies);
...
static void addrconf_dad_timer(unsigned long data)
{
	...
	ifp->timer.expires = jiffies + ifp->idev->nd_parms->retrans_time;
	add_timer(&ifp->timer);
}
			   ...
			   void *buffer, size_t *lenp)
{
	...
	if (write && *valp != val && valp != &ipv6_devconf_dflt.forwarding) {
		...
		if (*valp) {
			printk(KERN_DEBUG "Purge default routes(0)\n");
			start_bh_atomic();
			rt6_purge_dflt_routers(0);
			end_bh_atomic();
		}
#else
		printk(KERN_DEBUG "IPv6 forwarding is enabled, But not purge default routes\n");
#endif
	}
