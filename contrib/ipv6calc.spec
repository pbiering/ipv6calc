...
# $Id: ipv6calc.spec,v 1.135 2014/02/02 09:20:49 ds6peter Exp $
...
%{?_with_shared: %{expand: %%define enable_shared_library 1}}
...
%build
./configure --bindir=%{_bindir} --mandir=%{_mandir} %{?enable_ip2location:--enable-ip2location} %{?enable_geoip:--enable-geoip} %{?enable_ip2location_dyn:--with-ip2location-dynamic} %{?enable_geoip_dyn:--with-geoip-dynamic} %{?enable_shared:--enable-shared}
make clean
make
make test-minimal
