...
# Version    : $Id: test_ipv6loganon.sh,v 1.1 2007/01/30 17:00:37 peter Exp $
...
if [ "$1" != "bulk" ]; then
	...
	testscenarios_cache | grep -v "^#" | ./ipv6loganon -q $*
	retval=$?
	if [ $retval -ne 0 ]; then
		echo "Error executing 'ipv6loganon'!" >&2
		exit 1
	...
	testscenarios_cache | grep -v "^#" | ./ipv6loganon -q -n $*
	retval=$?
	if [ $retval -ne 0 ]; then
		echo "Error executing 'ipv6loganon'!" >&2
		exit 1
