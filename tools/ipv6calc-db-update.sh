...
# Version    : $Id: ipv6calc-db-update.sh,v 1.1 2015/02/15 20:19:37 ds6peter Exp $
# Copyright  : 2015-2015 by Peter Bieringer <pb (at) bieringer.de>
# License    : GNU GPL version 2
#
# Directory layout:
#  basedir/registries    downloaded files from registries
#         /db            generated Berkeley DB files


IPV6CALC_DAT_DIR="${IPV6CALC_DAT_DIR:-/var/local/share/ipv6calc/}"

IPV6CALC_DAT_DIR_REGISTRIES="${IPV6CALC_DAT_DIR}registries"
IPV6CALC_DAT_DIR_DB="${IPV6CALC_DAT_DIR}db"
...
if [ ! -e "$IPV6CALC_DAT_DIR_REGISTRIES" ]; then
	echo "NOTICE: missing registries subdirectory (try to create now): $IPV6CALC_DAT_DIR_REGISTRIES"
	mkdir $IPV6CALC_DAT_DIR_REGISTRIES
	if [ $? -ne 0 ]; then
		echo "ERROR : can't create registries subdirectory: $IPV6CALC_DAT_DIR_REGISTRIES ($!)"
		exit 1
	fi
elif [ ! -d "$IPV6CALC_DAT_DIR_REGISTRIES" ]; then
	echo "ERROR : existing, but not a directory: $IPV6CALC_DAT_DIR_REGISTRIES"
	exit 1
elif [ ! -w "$IPV6CALC_DAT_DIR_REGISTRIES" ]; then
	echo "ERROR : existing, but not a writable directory: $IPV6CALC_DAT_DIR_REGISTRIES"
	exit 1
fi

# db subdirectory
if [ ! -e "$IPV6CALC_DAT_DIR_DB" ]; then
	echo "NOTICE: missing db subdirectory (try to create now): $IPV6CALC_DAT_DIR_DB"
	mkdir $IPV6CALC_DAT_DIR_REGISTRIES
	if [ $? -ne 0 ]; then
		echo "ERROR : can't create db subdirectory: $IPV6CALC_DAT_DIR_DB ($!)"
		exit 1
	fi
elif [ ! -d "$IPV6CALC_DAT_DIR_DB" ]; then
	echo "ERROR : existing, but not a directory: $IPV6CALC_DAT_DIR_DB"
	exit 1
elif [ ! -w "$IPV6CALC_DAT_DIR_DB" ]; then
	echo "ERROR : existing, but not a writable directory: $IPV6CALC_DAT_DIR_DB"
	exit 1
fi

## update files from registries
if [ "$skip_download" != "1" ]; then
	echo "INFO  : updating files from registries now and store to: $IPV6CALC_DAT_DIR_REGISTRIES"
	$update_registries -D $IPV6CALC_DAT_DIR_REGISTRIES
	if [ $? -ne 0 ]; then
		echo "ERROR : update of files from registries resulted in a failure"
		exit 1
	fi
	echo "INFO  : update of files from registries finished in directory: $IPV6CALC_DAT_DIR_REGISTRIES"
else
...
for proto in 4 6; do
	echo "INFO  : call now check registries files for proto: IPv${proto} srcdir=$IPV6CALC_DAT_DIR_REGISTRIES dstdir=$IPV6CALC_DAT_DIR_DB"

	create_var="create_ipv${proto}"
	$check_registries -B -${proto} -C "${!create_var}" -S "$IPV6CALC_DAT_DIR_REGISTRIES" -D "$IPV6CALC_DAT_DIR_DB" $options
	rc=$?
	if [ $rc -ne 0 ]; then
